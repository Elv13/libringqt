...
/*DIALING      */  {{Call::State::INITIALIZATION, Call::State::ABORTED     , Call::State::ERROR        , Call::State::ERROR        ,  Call::State::ERROR        }},/**/
...
Call* Call::buildHistoryCall(const QMap<QStringRef,QStringRef>& hc)
{
   ...
   if (accId.isEmpty() && !printed) {
      qWarning() << "At least one history call has an invalid account identifier";
      printed = true;
   }
